...
import {
  ...
  OfflineSearchFilters,
} from "@/app/graphql/offlineMachineQuery";
...
export const useOfflineMachineStore = create<OfflineMachineState>(
  (set, get) => ({
    ...initialState,
    ...
    fetchData: async (filters, page, pageSize) => {
      ...
      try {
        ...
        const response = await fetchOfflineMachines(
          actualFilters,
          actualPageSize,
          offset
        );
        ...
        set({
          error: err instanceof Error ? err.message : "获取离线机器记录失败",
        });
      } finally {
        set({ loading: false });
      }
